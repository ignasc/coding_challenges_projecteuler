...
Each file can be run using node.js in a terminal and outputs are also presented in a terminal.
...
* [Problem #03](#problem-03) - largest prime factor of given number
* [Problem #04](#problem-04) - largest palindrome of two 3-digit products
* [Problem #05](#problem-05) - smallest number divisible by numbers 1 to 20
* [Problem #06](#problem-06) - difference between square of sum and sum of squares
* [Problem #07](#problem-07) - 10001st prime number
* [Problem #08](#problem-08) - 13 adjacent numbers with highest product in a number sequence
* [Problem #09](#problem-09) - product of a,b,c, when a^2+b^2+c^2=1000
* [Problem #10](#problem-10) - sum of all prime numbers below two million
* [Problem #11](#problem-11) - largest product of 4 adjacent numbers in a number grid
* [Problem #12](#problem-12) - triangular number with more than 500 divisors
* [Problem #13](#problem-13) - first 10 digits of the sum of 100-digit numbers
* [Problem #14](#problem-14) - longest Collatz sequence under one million
...
I had some options that I came up with myself beforehand. One was adding a number in each position that indicated, how many previous positions are there that let you arrive at the current position. The other was a reverse: how many directions I can go to from my current position. But I could not get past those two ideas further to make sense. I think the closest one to solution was the third option: to use bitwise operations. The idea was quite simple: ones represented a step to the right and zeroes represented a step down. When you have a square grid, regardless which path you take, you ALWAYS must take even number of steps to the right and down to get from top left to bottom right corner. So, for example, on a 2x2 grid one of the possible paths would be Right Down Right Down, which translates to 1010 or another one Right Right Down Down, which translates to 1100. So I figured that if I just generate every possible binary number, which had a length of 2 x grid size and just filter out unique ones, and then filter out all "false" results - that would leave me with all possible valid paths. The false results were either duplicates or numbers with uneven number of ones and zeroes. It seems to work, but, unfortunately, took way too long to calculate.
...
To get from point A to point B, you only have one way A->B, therefore B=1, same case getting from A to D, therefore D=1. So, to fill in the rest of points, you add up possible paths that take you to positions right before the current one you are calculating. To get from point A to point E, you can go either A->B->E or A->D->E, so a total of two possible paths. And to get that number, you simply add up the numbers of the vertices that are one step BEFORE position E: B+D=E, therefore E=1+1=2. If the point is on the edge (G), then there is nothing to the left of it, so you add 0: G=0+D=0+1=1. And so on until you get to the last bit: H=E+G=2+1=3, F=C+E+1+2=3 and finally X=F+H=3+3=6. All numbers filled in are below:
